...
def checkout(request: CheckoutRequest, db: Session = Depends(get_db)):
    return _generate_payment_link(
        request.product_id, request.final_discounted_price, db, request.buyer_username
    )
    ...
    while msg.tool_calls:
        for tool_call in msg.tool_calls:
            ...
            if func_name == "apply_discount_rule":
                ...
                result = _generate_payment_link(args["product_id"], args["final_price"], db)
...
def get_payment_link_invoice(payment_link_id: str, db: Session = Depends(get_db)):
    payment_link = db.query(PaymentLink).filter(PaymentLink.razorpay_link_id == payment_link_id).first()
    if payment_link is None:
...
def download_payment_link_invoice(payment_link_id: str, db: Session = Depends(get_db)):
    payment_link = db.query(PaymentLink).filter(PaymentLink.razorpay_link_id == payment_link_id).first()
    if payment_link is None:
        raise HTTPException(status_code=404, detail="Payment link not found")
...
def signup(request: SignupRequest, db: Session = Depends(get_db)):
    ...
    user = User(username=request.username, password=request.password, role="buyer")
    db.add(user)
    db.commit()
    db.refresh(user)
